...
LYFEBLOC_CORE_BIN=./lyfebloc-core
...
case "$1" in
   ...
   "init")
      if [ -f "$GENESIS_PATH" ]; then
          ...
          secrets=$("$LYFEBLOC_CORE_BIN" secrets init --num 4 --data-dir data- --json)
          echo "Secrets have been successfully generated"

          echo "Generating genesis file..."
          "$LYFEBLOC_CORE_BIN" genesis \
            --dir "$GENESIS_PATH" \
            --consensus ibft \
   ...
   *)
      ...
      echo "Executing lyfebloc-core..."
      exec "$LYFEBLOC_CORE_BIN" "$@"
      ;;
