...
build:
	...
	go build -o lyfebloc-core -ldflags="\
    	-X 'github/lyfeblocnetwork/lyfebloc-core/versioning.Version=$(LATEST_VERSION)' \
		-X 'github/lyfeblocnetwork/lyfebloc-core/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github/lyfeblocnetwork/lyfebloc-core/versioning.Branch=$(BRANCH)'\
		-X 'github/lyfeblocnetwork/lyfebloc-core/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/lyfebloc-core .
	env CORE_BINARY=${PWD}/artifacts/lyfebloc-core go test -v -timeout=30m ./e2e/...
